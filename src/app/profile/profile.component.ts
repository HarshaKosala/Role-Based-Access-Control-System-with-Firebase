...
export class ProfileComponent implements OnInit {
  ...
  ngOnInit() {
    this.route.params.subscribe((params: Params) => {
      ...
      this.as.$user.subscribe(user => {
        if (user) {
          this.as.afs
            .doc<Profile>(`profiles/${user.displayName}`)
            .valueChanges()
            .subscribe(profile => {
              if (profile) {
  ...
  updateArticleWriterName(profName: string) {
    const collection: AngularFirestoreCollection<
      Article
    > = this.as.afs.collection("articles", ref =>
      ref.where("writerName", "==", this.prof.name)
    );
